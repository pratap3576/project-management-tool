    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $events = array(
            ...
            "project_task_commented" => array(
                "notify_to" => array("mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "task_assignee", "task_collaborators", "team_members", "team"),
                "info" => $task_link
            ),
            ...
            "project_file_commented" => array(
                "notify_to" => array("mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "team_members", "team"),
                "info" => $project_file_link
            ),
            ...
            "project_customer_feedback_replied" => array(
                "notify_to" => array("mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "comment_creator", "team_members", "team"),
                "info" => $project_link
            ),
            "client_signup" => array(
            ...
            "project_task_deadline_pre_reminder" => array(
                "notify_to" => array("task_assignee", "team_members", "team"),
                "info" => $task_link
            ),
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        if ($notification->category == "ticket") {
            ...
            if ($notification->event == "estimate_request_received") {
                ...
                if ($notification->event == "estimate_sent") {
                    $email_template = $ci->Email_templates_model->get_final_template("estimate_sent");

                    $estimate_info = get_array_value($estimate_data, "estimate_info");
                    $primary_contact = $ci->Clients_model->get_primary_contact($estimate_info->client_id, true);
...
if (!function_exists('send_push_notifications')) {
    ...
    function send_push_notifications($event, $push_notify_to, $user_id = 0, $notification_id = 0) {
        ...
        if ($pusher_app_id && $pusher_key && $pusher_secret && $pusher_cluster) {
            require_once(APPPATH . "third_party/Pusher/init.php");
...
if (!function_exists('get_notification_multiple_tasks_data')) {
    ...
    function get_notification_multiple_tasks_data($tasks, $event) {
        ...
        foreach ($tasks as $task) {
            ...
            if ($task->assigned_to && $notify_to_users_from_settings_result->notify_to_terms == "task_assignee" && !in_array($task->assigned_to, $notify_to_users_from_settings)) {
